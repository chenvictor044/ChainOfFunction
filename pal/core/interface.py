...
from .backend import call_gpt, call_chat_gpt, call_gpt_history
...
class ProgramInterface:
    ...
    def generate_history(self, messages: list, temperature: float = 0.0, top_p: float =1.0, 
            max_tokens: int =512, majority_at: int =None, ):
        gens = call_gpt_history(messages, model=self.model, stop=self.stop, 
            temperature=temperature, top_p=top_p, max_tokens=max_tokens, majority_at=majority_at, )
        if self.verbose:
            print(gens)
        return gens
