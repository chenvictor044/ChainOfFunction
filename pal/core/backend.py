...
def call_gpt_history(messages, model='code-davinci-002', stop=None, temperature=0., top_p=1.0,
        max_tokens=128, majority_at=None):
    num_completions = majority_at if majority_at is not None else 1
    num_completions_batch_size = 5
...
def chat_api_history(model, max_tokens, stop, messages, temperature,
            top_p, n, best_of):
    ans = openai.chat.completions.create(
        model=model,
        max_tokens=max_tokens,
